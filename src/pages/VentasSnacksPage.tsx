...
export default function VentasSnacksPage() {
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      const [productData, paymentData, saleData] = await Promise.all([
        getSnackProducts(false),
        getMetodosPagoActivos(),
        getSnackSalesByDate(todayBogota()),
      ]);
      ...
      setError(e?.message || "No fue posible cargar el módulo de ventas.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter((product) => !q || [product.numero_producto, product.nombre_producto].some((value) => value.toLowerCase().includes(q)));
  }, [products, search]);
  ...
  const sell = async () => {
    ...
    setSuccess("");
    try {
      await registerSnackSale(cartItems.map(({ product, cantidad }) => ({ id_producto: product.id_producto, cantidad })), medioPago);
      setCart({});
      setSuccess(`Venta registrada por ${money(total)}. El inventario fue descontado automáticamente.`);
      await load(true);
    } catch (e: any) {
  ...
  return (
    <div className="snack-page">
      <div className="snack-page-head">
        <div><h1>Ventas de snacks</h1><p>Registra ventas y descuenta automáticamente las existencias del inventario.</p></div>
        <button className="snack-btn secondary" onClick={() => load(true)} disabled={refreshing}><RefreshCw size={16} className={refreshing ? "spin-icon" : ""} /> Actualizar</button>
      </div>
      ...
      <div className="snack-kpis">
        <div><span>Ventas de hoy</span><b>{sales.length}</b></div>
        <div><span>Recaudo snacks hoy</span><b>{money(totalToday)}</b></div>
      ...
      <div className="snack-sales-layout">
        <section className="snack-card">
          <div className="snack-toolbar"><div className="snack-search"><Search size={16} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar snack…" /></div></div>
          <div className="snack-product-grid">
            {loading ? <div className="snack-empty">Cargando productos…</div> : filtered.length === 0 ? <div className="snack-empty">No hay productos disponibles.</div> : filtered.map((product) => {
              const quantity = cart[product.id_producto] ?? 0;
              return <article key={product.id_producto} className={`snack-sale-product ${product.cantidad <= 0 ? "sold-out" : ""}`}>
                <div className="snack-sale-product-head"><span>{product.numero_producto}</span><span className={`snack-stock ${product.cantidad <= 5 ? "low" : ""}`}>{product.cantidad} disp.</span></div>
        ...
        <aside className="snack-card snack-cart-card">
          <div className="snack-card-title"><div><ShoppingCart size={18} /><strong>Venta actual</strong></div><span>{units} unidad{units === 1 ? "" : "es"}</span></div>
          <div className="snack-cart-list">
            {cartItems.length === 0 ? <div className="snack-empty">Aún no has agregado productos.</div> : cartItems.map(({ product, cantidad }) => (
              <div className="snack-cart-row" key={product.id_producto}>
                ...
                <div><b>{money(product.precio * cantidad)}</b><button className="snack-icon-btn" onClick={() => setQuantity(product, 0)} title="Quitar"><Trash2 size={14} /></button></div>
              </div>
            ))}
          </div>
          <div className="snack-cart-total"><span>Total</span><b>{money(total)}</b></div>
          <label className="snack-payment-label">Método de pago *<select value={medioPago} onChange={(e) => setMedioPago(e.target.value)}><option value="">Seleccionar</option>{metodos.map((method) => <option key={method} value={method}>{method}</option>)}</select></label>
          <button className="snack-btn primary wide" disabled={saving || !cartItems.length} onClick={sell}>{saving ? "Registrando venta…" : `Cobrar ${money(total)}`}</button>
        </aside>
      </div>

      <section className="snack-card">
        <div className="snack-card-title"><div><strong>Ventas registradas hoy</strong></div><span>{todayBogota()}</span></div>
        <div className="snack-table-wrap">
          <table className="snack-table">
            ...
            <tbody>{sales.length === 0 ? <tr><td colSpan={5} className="snack-empty">Todavía no hay ventas de snacks hoy.</td></tr> : sales.map((sale) => <tr key={sale.id_venta}><td>{timeBogota(sale.fecha_venta)}</td><td>{sale.items.map((item) => `${item.cantidad}× ${item.nombre_producto}`).join(", ")}</td><td>{sale.medio_pago}</td><td>{sale.vendedor_email || "—"}</td><td><strong>{money(sale.total)}</strong></td></tr>)}</tbody>
          </table>
        </div>
      </section>
    </div>
