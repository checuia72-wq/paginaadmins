...
import CarteraPendienteModal from "../components/admin/CarteraPendienteModal";
import SnackAnalyticsDashboard from "../components/admin/SnackAnalyticsDashboard";
...
export default function OverviewPage() {
  ...
  return <div className="crm-dashboard">
    ...
    <SnackAnalyticsDashboard fromDate={fromDate} toDate={toDate} />

    <CarteraPendienteModal open={carteraOpen} onClose={()=>setCarteraOpen(false)} reservas={filteredReservas} planes={data.planes} pagos={data.pagos} devoluciones={data.devoluciones}/>
  </div>;
}

function Kpi({ icon, label, value, helper, tone, onClick }: { icon: React.ReactNode; label: string; value: string; helper: string; tone: string; onClick?:()=>void }) { return <div className={`crm-kpi crm-kpi-${tone}${onClick?" crm-kpi-clickable":""}`} onClick={onClick} onKeyDown={onClick?(event)=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();onClick();}}:undefined} role={onClick?"button":undefined} tabIndex={onClick?0:undefined}><div className="crm-kpi-top"><div className="crm-kpi-icon">{icon}</div><span>{label}</span></div><strong>{value}</strong><small>{helper}</small></div>; }