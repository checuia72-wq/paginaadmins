...
function buildControlOperativoRows(
  reservas: any[],
  participantes: any[],
  planes: any[],
  fechas: any[],
  ...
  const fechaMap=new Map<number,any>();
  const horaMap=new Map<number,any>();
  const horasPorPlan=new Map<number,any[]>();

  for(const f of fechas as any[]){
    for(const id of [f.id_fecha,f.id_plan_fecha,f.id]) if(id!=null) fechaMap.set(Number(id),f);
  }
  ...
  for(const r of reservas as any[]){
    const planId=r.id_plan==null?null:Number(r.id_plan);
    const plan=planId==null?null:planMap.get(planId);
    const fechaId=r.id_fecha??r.id_plan_fecha??r.id_fecha_reserva;
    const horaIdOriginal=r.id_hora??r.id_plan_hora??r.id_hora_reserva;
    const fechaRelacionada=fechaMap.get(Number(fechaId));
    let horaRelacionada=horaMap.get(Number(horaIdOriginal));
    let horaId=horaIdOriginal;
    ...
    const cantidadPersonas=r.cantidad_personas==null?personas.filter(Boolean).length:num(r.cantidad_personas);
    const precioPlan=num(plan?.precio_plan);
    const totalGuardado=num(r.valor_total)>0?num(r.valor_total):num(r.precio_unitario)*cantidadPersonas;
    const totalCalculado=precioPlan>0?precioPlan*cantidadPersonas:0;
    const total=totalGuardado>0?totalGuardado:totalCalculado;
    ...
    for(const p of personas){
      ...
      rows.push({
        ...
        id_codigo_operativo:r.id_codigo_operativo==null?null:Number(r.id_codigo_operativo),
        incluye_almuerzo:!!r.incluye_almuerzo,
        id_plan:planId,
        id_fecha:fechaId==null?null:Number(fechaId),
        id_hora:horaId==null?null:Number(horaId),
        ...
        estado_operativo_at:text(r.estado_operativo_at),
      });
    }
  }
...
export async function getControlOperativo():Promise<ControlOperativoRow[]>{
  ...
  return buildControlOperativoRows(
    reservasRes.data??[],
...
export async function getControlOperativoCoordinador():Promise<ControlOperativoRow[]>{
  const current=await getCurrentRole();
  if(current?.role!=="coordinador") return getControlOperativo();

  const {data,error}=await client().rpc("control_operativo_coordinador_data");
  if(error) throw error;

  const raw:any=data??{};
  return buildControlOperativoRows(
    Array.isArray(raw.reservas)?raw.reservas:[],
    Array.isArray(raw.participantes)?raw.participantes:[],
    Array.isArray(raw.planes)?raw.planes:[],
    Array.isArray(raw.fechas)?raw.fechas:[],
    Array.isArray(raw.horas)?raw.horas:[],
  );
}
